-- ============================================================================
-- Admin panel: global "zweryfikowane" checkmark on questions
-- ----------------------------------------------------------------------------
-- 2026-08-17. The owner reviews each question together with its smaczki once
-- and marks it done, so the list can hide it from the review queue.
--
--   * questions.verified_at — NULL = not verified. Kept OUTSIDE
--     question_snapshot() / question_content_hash() on purpose (same as
--     en_review_needed): toggling never invalidates open drafts.
--   * admin_toggle_verified(question_id) — flips the flag, returns the new
--     state (true = verified). Global, not per-admin (unlike favorites):
--     "verified" is a property of the content, not of the viewer. Both
--     directions are audit-logged (verified / unverified).
--   * admin_list_questions gains is_verified + p_only_unverified filter
--     (return-type change => drop + recreate + re-grant).
--   * admin_get_question exposes verified_at.
--
...
-- 2) Toggle RPC
-- ----------------------------------------------------------------------------
create or replace function public.admin_toggle_verified(p_question_id uuid)
...
begin
  ...
  update public.questions
     set verified_at = case when verified_at is null then now() else null end
   where id = p_question_id
  returning verified_at into v_new;
  ...
  values (p_question_id, case when v_new is not null then 'verified' else 'unverified' end, auth.uid());

  return v_new is not null;
end;
$$;

revoke all on function public.admin_toggle_verified(uuid) from public, anon;
grant execute on function public.admin_toggle_verified(uuid) to authenticated, service_role;

-- ----------------------------------------------------------------------------
-- 3) admin_get_question: expose verified_at (outside the snapshot => hash-stable)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) admin_list_questions: is_verified column + p_only_unverified filter
--    (replaces the 20260814120000_admin_question_favorites.sql version)
-- ----------------------------------------------------------------------------
drop function if exists public.admin_list_questions(text, boolean, integer, integer, boolean, boolean);
...
create function public.admin_list_questions(
  ...
  en_review_needed boolean, is_favorite boolean, is_verified boolean, total_count bigint
...
revoke all on function public.admin_list_questions(text, boolean, int, int, boolean, boolean, boolean) from public, anon;
grant execute on function public.admin_list_questions(text, boolean, int, int, boolean, boolean, boolean) to authenticated, service_role;
