...
/**
 * The full question editor. Used in two places:
 *  - the /q/[id] route (standalone page, also handles /q/new),
 *  - inline on the list page, expanded underneath the clicked row.
 *
 * All navigation goes through callbacks so the component itself is
 * router-agnostic: onBack (Wróć/Zwiń), onCreated(newId) after a create is
 * published, onAfterDelete after a delete is approved, onChanged after any
 * successful write (the list uses it to refresh its rows), onDirtyChange so
 * the parent can warn before collapsing unsaved edits.
 */
export default function QuestionEditor({
  questionId, inline = false,
  onBack, onCreated, onAfterDelete, onChanged, onDirtyChange,
}) {
  const id = questionId;
  const isNew = id === 'new';
  ...
  // Let the parent (inline list) know, so collapsing warns about unsaved edits.
  ...
  const publish = () => run(async () => {
    ...
    if (isNew) { onCreated?.(res.question_id); return res; }
    await refresh();
    onChanged?.();
    return res;
  }, 'Opublikowane.');
  ...
  const approve = () => run(async () => {
    ...
    if (res.action === 'delete') { onChanged?.(); onAfterDelete?.(); return res; }
    if (isNew && res.question_id) { onCreated?.(res.question_id); return res; }
    await refresh();
    onChanged?.();
    return res;
